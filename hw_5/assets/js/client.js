...
async function getMessagesList(sort, skip, limit, sortValue) {
    const res = await fetch(`/messages?sort=${sort}&skip=${skip}&limit=${limit}&sortValue=${sortValue}`);
...
function renderMessage(msg, parent) {
    ...
    p_date.innerText = new Date(msg.date).toISOString();
...
function renderEditMessageForm(parent) {
    ...
    input.dataset["date"] = parent.dataset.date;
